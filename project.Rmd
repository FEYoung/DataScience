...
Following on from their research this analysis performs the same analysis using the boosting machine learning algorithm rather than best fit Random Forest approach used by Velloso (2013).

The predictive ability of training Model One generated an overall accuracy was 0.96 and removing eight zero influence variables generated in Model Two reduced the overall accuracy to 0.95.  Model One was used to evaluate the prediction accuracy information located in the validation dataset.  Model One correctly identified all 20 validation cases.

##----------------------------------------------

###INTRODUCTION

The six male test subjects were of 20 - 28 years of age and inexperienced in weight lifting exercises.  The dumbbell weighed 1.25 kg.
...
what analysis did I do?  what were the results?
...
The dataset consists of 19622 rows with 160 columns.  Exploration of the dataset concluded that 106 columns could be removed as they contained no valid information.  These columns contained the words or abbreviations: kurtosis, mean, stddev, var, var_total, avg, skewness, max, min, new_window, num_window, amplitude.  There are no columns with missing data or zero variance to deal with.

```{R removing columns & splitting dataset}
...
It was decided to split the dataset into two randomly selected pieces using the createDataPartition command because of the large dataset size.  RAM (random access memory) requirements and computational time were also contributing factors.  The two pieces: 60% (11767 rows) for model training and 40% (7846 rows) for model testing were chosen as trial and error with the training model provided evidence that model accuracy increased as the size of training dataset was increased.
...
The choice of training model was related to its computational RAM (random access memory) expense, the time available to complete the module project analysis submission and the defined accuracy of the model compared to others as described by Jeff Leek in the video lecture on Boosting.  This led to the selection of the boosting model - command "gbm".  

Preprocessing of the training dataset was performed at the same time as model training and it centred and scaled the all variables.  If any other preprocessing commands were added the computer produced a BSoD (blue screen of death).
...
On completion of the model it was noted that eight variables had no model influence and were removed from the training dataset.  The variables are accel_belt_x, accel_belt_y, pitch_arm, gyros_arm_z, accel_arm_y, yaw_dumbbell, yaw_forearm and gyros_forearm_y.

NOTE: The coding below has been left in situ but not run as the html output file could not be created otherwise.
...
The model was run again without these variables for the purpose of cross validation with 50% of the original training dataset rows.  Is the model accuracy improved without these eight variables?
	...
	##12 - rerunning the gbm model - version 2
	#cache = TRUE
...
The results from the training the model and its predictive accuracy on the testing dataset.
...
NOTE: The coding below has been left in situ but not run as the html output file could not be created otherwise.  The output graphs numbered 4 to 6 can be viewed in the github repository.
...
Running the training model without the eight variables reduced the overall accuracy of the model from 0.96 to 0.95.  The PPV was reduced for four of the classes to 0.94 but class B reduced to 0.91.  For NPV all class values were above 0.98.  As the accuracy of the model dropped without these eight predictors the first model was chosen to make predictions for the validation dataset.

The prediction of class for each of the 20 rows in the validation dataset is:
...
Of the 20 rows in the validation dataset all 20 have been correctly predicted.
