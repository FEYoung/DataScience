...
```{r plotting histogram, fig.width = 10}
	##1 - plotting the histogram
	with (totalsteps, {
	plot(date, steps, main = "Total number of steps taken each day with NA values present", type = "h", xlab = "Date", ylab = "Number of steps", lwd = 14)
	})
```
...
```{r missing}	
	...
	countnasubset <- sum(is.na(valuesna))
	countnasubset
```
...
```{r part 3, fig.width = 10}
	...
	with (finalna, {
	plot(date, steps, main = "Total number of steps taken each day with NA values replaced", type = "h", xlab = "Date", ylab = "Number of steps", lwd = 14)
	})
...
```{r week days, fig.height = 8, fig.width = 10}	
			...
			day <- weekdays(weekends$date, abbreviate = TRUE)
				...
				daydate <- cbind(weekends, day)
					...
					daydate <- daydate[c(2:4)]
						...
						weekdayorend <- dcast(meltweekends, interval + day ~ variable, mean)
	...
	weekdayorend$daytype <- 0
		...
		for (i in numberrow) {
			...
			weekdayorend <- weekdayorend[c(1, 3, 4)]
				...
				weekdayorend <- dcast(melting, interval + daytype ~ variable, mean)
		
	##4.11 - plotting the final graph
	graph <- qplot(interval, steps, data = weekdayorend, facets = daytype~., xlab = "Interval (hhmm)", ylab = "Number of steps", main = "Is there a difference between the mean values \n for weekday and weekend \n activity for each interval period? \n")
	graph + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 3.5))
```

##generating the html output file
render("PA1_template_corrected.Rmd", html_document())
